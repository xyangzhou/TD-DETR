...
class Transformer(nn.Module):
    ...
    def __init__(self, d_model=512, nhead=8, num_queries=2, num_encoder_layers=6,
        ...
        t2v_encoder_layer = T2V_TransformerEncoderLayer(d_model, nhead, dim_feedforward,
                                                dropout, activation, normalize_before, beta)
    ...
    def forward(self, src, mask, query_embed, pos_embed, video_length=None, sof_token=None):
        """
        Args:
            src: (batch_size, L, d)
            mask: (batch_size, L)
        ...
        src = self.t2v_encoder(src, src_key_padding_mask=mask, pos=pos_embed, video_length=video_length, sof_token=sof_token)  # (L, batch_size, d)
...
class T2V_TransformerEncoderLayer(nn.Module):
    ...
    def __init__(self, d_model, nhead, dim_feedforward=2048, dropout=0.1,
                 activation="relu", normalize_before=False, beta=0.7):
        super().__init__()
        self.beta = beta

        self.self_attn = nn.MultiheadAttention(d_model, nhead, dropout=dropout)
        self.self_attn_flow = nn.MultiheadAttention(d_model, nhead, dropout=dropout)
        # Implementation of Feedforward model
        self.linear1 = nn.Linear(d_model, dim_feedforward)
        self.dropout = nn.Dropout(dropout)
        self.linear2 = nn.Linear(dim_feedforward, d_model)
        from td_detr.model import LinearLayer
        self.flow_proj = nn.Sequential(*[
            LinearLayer(d_model, d_model, layer_norm=True),
            LinearLayer(d_model, d_model, layer_norm=True),
            # LinearLayer(d_model, d_model, layer_norm=True),
        ])
    ...
    def forward_post(self,
        ...
        tmp_video = torch.cat([sof_token.permute(1, 0, 2), q], dim=0)
        flow = tmp_video[1:]
        flow = flow - tmp_video[:-1]
        flow_mask = src_key_padding_mask[:, 2:video_length + 1]
        flow_mask = torch.cat([torch.zeros((flow_mask.shape[0], 1), device=flow_mask.device, dtype=flow_mask.dtype), flow_mask], dim=1).unsqueeze(2)

        flow = self.flow_proj(flow)
        attn_mask_flow = torch.matmul(flow_mask.float(), kmask.float()).bool().repeat(self.nhead, 1, 1)
        f = self.self_attn_flow(flow, k, value=v, attn_mask=attn_mask_flow,
                                key_padding_mask=src_key_padding_mask[:, video_length + 1:])[0]

        # src2 = beta * src2 + (1 - beta) * f
        src2 = self.beta * src[1:video_length + 1] + (1 - self.beta) * f + self.dropout1(src2)
