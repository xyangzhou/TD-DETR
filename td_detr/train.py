...
from td_detr.config import BaseOptions
from td_detr.start_end_dataset import \
    StartEndDataset, start_end_collate, prepare_batch_inputs, SyntheticDataset, SimSampler, SimilarityConcatDataset
from td_detr.inference import eval_epoch, start_inference, setup_model
...
def train_epoch(model, criterion, train_loader, optimizer, opt, epoch_i, tb_writer):
    ...
    # print/add logs
    save_train_meta_interval = 20
    if epoch_i == 0 or epoch_i % save_train_meta_interval == 0:
        save_jsonl(all_meta, f"{opt.results_dir}/train_meta_{epoch_i}.jsonl")
...
def train(model, criterion, optimizer, lr_scheduler, train_dataset, val_dataset, opt):
    ...
    train_loader = DataLoader(
        ...
        batch_sampler=SimSampler(sampler, opt.bsz, False, sim_matrix=os.path.join(opt.sim_matrix_path, f'{opt.dset_name}_similarity_matrix_top50.pt'), sample_strategy=opt.sample_strategy)
    )
    ...
    for epoch_i in trange(start_epoch, opt.n_epoch, desc="Epoch"):
        ...
        if opt.eval_path is not None and (epoch_i + 1) % eval_epoch_interval == 0:
            ...
            if opt.dset_name in ['hl']:
                ...
                stop_score = metrics["brief"]["MR-full-R1@0.7"] / 2
...
def start_training():
    ...
    train_dataset_1 = SyntheticDataset(**dataset_config, alpha=opt.alpha)
    train_dataset_2 = StartEndDataset(**dataset_config)
    train_dataset = SimilarityConcatDataset([train_dataset_1, train_dataset_2])
