...
from td_detr.config import TestOptions
from td_detr.model import build_model
from td_detr.span_utils import span_cxw_to_xx
from td_detr.start_end_dataset import StartEndDataset, start_end_collate, prepare_batch_inputs
from td_detr.postprocessing_td_detr import PostProcessorDETR
...
def compute_mr_results(model, eval_loader, opt, epoch_i=None, criterion=None, tb_writer=None):
    ...
    if opt.dset_name in ['charades', 'charades_cg']:
        if opt.v_feat_dim == 4096: # vgg
            post_processor = PostProcessorDETR(
                clip_length=2, min_ts_val=0, max_ts_val=360,
        ...
        post_processor = PostProcessorDETR(
            clip_length=opt.clip_length, min_ts_val=0, max_ts_val=1500,
            min_w_l=2, max_w_l=1500, move_window_method="left",
            process_func_names=("clip_ts", "round_multiple")
        )
    elif opt.dset_name in ['ego4d']:
        post_processor = PostProcessorDETR(
            clip_length=opt.clip_length, min_ts_val=0, max_ts_val=1500,
            min_w_l=2, max_w_l=1500, move_window_method="left",
            process_func_names=("clip_ts", "round_multiple")
...
def eval_epoch(model, eval_dataset, opt, save_submission_filename, epoch_i=None, criterion=None, tb_writer=None):
    ...
    if opt.dset_name in ['charades', 'tacos', 'charades_cg', 'ego4d']:
        new_submission = []
        for s in submission:
