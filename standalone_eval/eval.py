...
def compute_mr_ap(submission, ground_truth, iou_thds=np.linspace(0.5, 0.95, 10),
    ...
    if num_workers > 1:
        ...
        for data_triple in data_triples:
            ...
            qid2ap_list[qid] = scores
    # with open("qid2ap_list.json", "w") as f:
    #     for i in qid2ap_list:
    #         qid2ap_list[i] = qid2ap_list[i].tolist()
    #     json.dump(qid2ap_list, f)
    ...
    if ap_array.shape[0] == 0:  # no prediction
        ap_array = np.zeros((1, len(iou_thds)))
...
def compute_mr_r1(submission, ground_truth, iou_thds=np.linspace(0.5, 0.95, 10)):
    ...
    if len(qids) == 0:
        pred_windows = np.zeros((1, 2))
        gt_windows = np.zeros((1, 2))
    pred_gt_iou = compute_temporal_iou_batch_paired(pred_windows, gt_windows)
    # with open("IoU_list.json", "w") as f:
    #     json.dump({k: v for k, v in zip(qids, pred_gt_iou.tolist())}, f)
...
def compute_mr_miou(submission, ground_truth):
    """If a predicted segment has IoU >= iou_thd with one of the 1st GT segment, we define it positive"""
    pred_qid2window = {d["qid"]: d["pred_relevant_windows"][0][:2] for d in submission}  # :2 rm scores
    # gt_qid2window = {d["qid"]: d["relevant_windows"][0] for d in ground_truth}
    gt_qid2window = {}
    ...
    if len(qids) == 0:
        ...
        gt_windows = np.zeros((1, 2))
    pred_gt_iou = compute_temporal_iou_batch_paired(pred_windows, gt_windows)
    iou_thd2recall_at_one = float(f"{np.mean(pred_gt_iou) * 100:.2f}")
    return iou_thd2recall_at_one
...
def eval_moment_retrieval(submission, ground_truth, verbose=True):
    ...
    for l_range, name in zip(length_ranges, range_names):
        ...
        windows = []
        query = []
        for gg in _ground_truth:
            q = gg["query"]
            q = q.split()
            query.append(len(q))
            for g in gg["relevant_windows"]:
                windows.append(g[1] - g[0])
        print(name + '_video: {}'.format(np.mean(windows)))
        print(name + '_text: {}'.format(np.mean(query)))
        iou_thd2average_precision = compute_mr_ap(_submission, _ground_truth, num_workers=8, chunksize=50)
        iou_thd2recall_at_one = compute_mr_r1(_submission, _ground_truth, np.linspace(0.1, 1.0, 10))
        miou = compute_mr_miou(_submission, _ground_truth)
        ret_metrics[name] = {"MR-mAP": iou_thd2average_precision, "MR-R1": iou_thd2recall_at_one, "MR-mIOU": miou}
        if verbose:
...
def eval_submission(submission, ground_truth, verbose=True, match_number=True):
    ...
    if "pred_relevant_windows" in submission[0]:
        ...
        moment_ret_scores_brief = {
            ...
            "MR-full-R1@0.9": moment_ret_scores["full"]["MR-R1"]["0.9"],
